...
#define J1939_STARTING_ADDRESS_1 0x56  //充电桩地址
...
void J1939_CAN_Transmit(J1939_MESSAGE *MsgPtr)
{
	...
	switch (Can_Node)
	{
		case  Select_CAN_NODE_1:
		{
			...
			//CAN硬件开始发送数据
		    pPara->TXmod = CAN_A;
		    pPara->TXobjID = CTRL_TX_OBJ;
		    pPara->txLen = MsgPtr->Mxe.DataLength;
...
int J1939_CAN_Receive(J1939_MESSAGE *MsgPtr)
{
	...
	switch (Can_Node)
	{
		case  Select_CAN_NODE_1:
		{
			...
			if(CAN_readMesID(pPara))//判断CAN硬件1是否有数据到来
			{
				//你的代码，从CAN硬件1 中将数据读取后，存入 MsgPtr
			    MsgPtr->CanId =  pPara->rxMsgID;
			    MsgPtr->Mxe.DataLength = pPara->rxSize;
			    memcpy(MsgPtr->Mxe.Data,pPara->rxmsgData,MsgPtr->Mxe.DataLength);
				return 1;
			}
			return 0;
			break;
