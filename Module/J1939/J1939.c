...
void J1939_TP_Poll()
{
	...
	if(J1939_TP_Flags_t.state == J1939_TP_RX)
	{
		...
		case J1939_TP_RX_DATA_WAIT:
			/*等待TP.DT帧传输的消息*/
			if(J1939_TP_TIMEOUT_ABNORMAL == J1939_TP_RX_RefreshCMTimer(J1939_TP_FREQ))
			{
				/* 等待超时，发生连接异常，跳转到异常步骤 */
				TP_RX_MSG.state = J1939_TP_RX_ERROR;
			}
	...
	if(J1939_TP_Flags_t.state == J1939_TP_TX)
	{
		...
		switch (TP_TX_MSG.state)
		{
			...
			case J1939_TP_TX_CM_WAIT:
	    		/*等待TP.CM_CTS帧传输的消息*/
				if(J1939_TP_TIMEOUT_ABNORMAL == J1939_TP_TX_RefreshCMTimer(J1939_TP_FREQ))
				{
					/* 等待超时，发生连接异常，跳转到异常步骤 */
					TP_TX_MSG.state = J1939_TP_TX_ERROR;
				}
